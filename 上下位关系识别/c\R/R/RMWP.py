...
def xia(en_model, positive_projections):
    # 获取所有词汇和对应的向量
    ...
    for word, vector in all_vectors.items():
        ...
        for i in range(n_clusters):
            pos_concat[i] = np.matmul(positive_projections[i], vector)
        xia_vectors[word] = pos_concat.reshape(n_embeddings * n_clusters, order='C')
    return xia_vectors

def shang(en_model, positive_projections):
    # 获取所有词汇和对应的向量
    all_vectors = {word: en_model[word] for word in en_model.key_to_index.keys()}
    shang_vectors = {}
    # 打印所有的词汇及其向量
...
def xunlina(zheng, fu,top_k):
    ...
    net = MLPClassifier(solver='adam', alpha=1e-5)
