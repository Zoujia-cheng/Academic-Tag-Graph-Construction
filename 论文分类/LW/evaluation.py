...
json_file_path = r'MAGCS/MAGCS_paper.json'
tit=[]
data=[]
ff=[]
...
with open(json_file_path, 'r',encoding="utf-8") as json_file:
    for line in json_file:
        ...
        i = json.loads(line)
        paper_number = i["label"]
        tit.append(len(paper_number))
t=list(set(tit))
print(t)
for k in t:
	print(k)
	preds = []
	targets = []
	with open(f'MAGCS/data' + str(k) + '.json', encoding='utf-8') as fin1, \
			open(f'MAGCS/data' + str(k)+ str(k) + str(k) + '.json', encoding='utf-8') as fin2:
	...
	train_labels = []
	with open(f'MAGCS/data' + str(k) + '.json', encoding='utf-8') as fin:
		for line in fin:
			data = json.loads(line)
			train_labels.append(data['label'])
	inv_w = get_inv_propensity(mlb.transform(train_labels), 0.55, 1.5)
	psp1, psp3, psp5, psn3, psn5 = get_psp_1(preds, targets, inv_w, mlb), get_psp_3(preds, targets, inv_w,
																					mlb), get_psp_5(preds, targets,
																									inv_w, mlb), \
