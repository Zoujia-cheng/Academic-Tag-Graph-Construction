...
parser.add_argument('--dataset', default='New_data')
parser.add_argument('--model', default='specter')
parser.add_argument('--full_text', type=int, default=0)
...
with open(f'{dataset}/{dataset}_paper.json') as fin, open(f'{dataset}/{dataset}_paper_emb.txt', 'w') as fout:
	for line in tqdm(fin):
		...
		if args.full_text == 1:
			...
			for x in data['sec']:
				section = x['section']
				if section not in sec2emb2:
					sec2emb2[section] = np.zeros(768)
					sec2cnt2[section] = 0.0
			...
			for x in data['paragraphs']:
				...
				sec2emb[section] += sec2emb[section]
				sec2cnt[section] += sec2cnt[section]

			body_emb = np.zeros(768)
			for section in sec2emb:
				body_emb += np.divide(sec2emb[section], sec2cnt[section], where=sec2cnt[section]!=0)#sec2emb[section] / sec2cnt[section]
			if len(sec2emb) > 0:
