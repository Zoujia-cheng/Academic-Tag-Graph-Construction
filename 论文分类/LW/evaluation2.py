...
parser.add_argument('--model', default='futex', type=str)
...
preds = []
targets = []
with open(f'{dataset}/{dataset}_paper.json',encoding="utf-8") as fin1, \
	 open(f'{dataset}/{dataset}_predictions_futex.json',encoding="utf-8") as fin2:
...
print('P@1:', p1, ', ', \
	...
	'NDCG@3:', n3, ', ', \
	'NDCG@5:', n5)
# with open('bert-base-chinese/scores.txt', 'a') as fout:
# 	fout.write('{:.4f}'.format(p1)+'\t'+'{:.4f}'.format(p3)+'\t'+'{:.4f}'.format(p5)+'\t'+ \
# 			'{:.4f}'.format(n3)+'\t'+'{:.4f}'.format(n5)+'\n')

train_labels = []
with open(f'{dataset}/{dataset}_paper.json',encoding="utf-8") as fin:
	for line in fin:
