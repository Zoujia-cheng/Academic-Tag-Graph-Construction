...
def load_vocabulary(path):
    ...
    print("load vocab from: {}, containing words: {}".format(path, vocab))
    w2i = {}
    i2w = {}
...
####################################################################
####### DataProcessor 6: for Multitask-LSTM-WordLevelFeature #######
####################################################################

class DataProcessor_bert_wlf_wol(object):
    def __init__(self,
                 input_seq_path,
                 input_seq_word_path,
                 output_seq_bio_path,
        ...
        #     bios = []
        #     attrs = []
        #     for word, w, bio, attr in zip(line1.split(" "), line4.split(" ") , line2.split(" "), line3.split(" ")):
        #         if word != "[SPA]" and w != "[SPA]":
        #             words.append(word)
        #             ww.append(w)
        #             bios.append(bio)
        #             attrs.append(attr)
        ...
        #     inputs_seq.append(seq)
        #
        #     seq = [w2i_word[w] if w in w2i_word else w2i_word["[UNK]"] for w in ww]
        #     print(seq)
        #     inputs_seq_word.append(seq)
        #
        #     bios.insert(0, "O")
        ...
        with open(input_seq_path, "r", encoding="utf-8") as f:
            for line in f.read().strip().split("\n"):
                ...
                k.insert(0, "[CLS]")
                k.append("[SEP]")
                seq = [w2i_char[word] if word in w2i_char else w2i_char["[UNK]"] for word in k]
                inputs_seq.append(seq)
        ...
        with open(output_seq_bio_path, "r", encoding="utf-8") as f:
            for line in f.read().strip().split("\n"):
                ...
                k.insert(0, "O")
                k.append("O")
                seq = [w2i_bio[word] for word in k]
                outputs_seq_bio.append(seq)
        ...
        with open(output_seq_attr_path, "r", encoding="utf-8") as f:
            for line in f.read().strip().split("\n"):
                ...
                k.insert(0, "null")
                k.append("null")
                seq = [w2i_attr[word] for word in k]
                outputs_seq_attr.append(seq)

        assert len(inputs_seq) == len(outputs_seq_bio)
        assert all(len(input_seq) == len(output_seq_bio) for input_seq, output_seq_bio in zip(inputs_seq, outputs_seq_bio))
        assert len(inputs_seq) == len(outputs_seq_attr)
        assert all(len(input_seq) == len(outputs_seq_attr) for input_seq, outputs_seq_attr in zip(inputs_seq, outputs_seq_attr))
    ...
    def get_batch(self, batch_size):
        ...
        max_seq_len = max(lens)
        for input_seq, input_seq_word, input_mask, input_segment, output_seq_bio, output_seq_attr, l in zip(inputs_seq_batch,inputs_seq_word_batch,
                                                                                            inputs_mask_batch,
                                                                                            inputs_segment_batch,
                                                                                            outputs_seq_bio_batch,
                                                                                            outputs_seq_attr_batch,
            ...
            input_seq.extend([self.w2i_char["[PAD]"]] * (max_seq_len - l))
            input_seq_word.extend([self.w2i_word["[PAD]"]] * (max_seq_len - l))
            input_mask.extend([0] * (max_seq_len - l))
            input_segment.extend([0] * (max_seq_len - l))
            output_seq_bio.extend([self.w2i_bio["O"]] * (max_seq_len - l))
            output_seq_attr.extend([self.w2i_attr["null"]] * (max_seq_len - l))

            padded_sequences = [seq + [0] * (max_seq_len - len(seq)) for seq in inputs_seq_word_batch]

        return (
                np.array(inputs_seq_batch, dtype="int32"),
                np.array(padded_sequences, dtype="int32"),
                np.array(inputs_mask_batch, dtype="int32"),
                np.array(inputs_segment_batch, dtype="int32"),
                np.array(outputs_seq_bio_batch, dtype="int32"),
                np.array(outputs_seq_attr_batch, dtype="int32"))
...
####################################################################
####### DataProcessor 7: for BERT-WordLevelFeature #######
####################################################################

class bert_wlf_wol(object):
    def __init__(self,
                 input_seq_path,
                 input_seq_word_path,
                 output_seq_bio_path,
    ...
    def get_batch(self, batch_size):
        ...
        max_seq_len = max(lens)
        for input_seq, input_seq_word, input_mask, input_segment, output_seq_bio,l in zip(inputs_seq_batch,inputs_seq_word_batch,
                                                                                            inputs_mask_batch,
                                                                                            inputs_segment_batch,
                                                                                            outputs_seq_bio_batch,
                                                                                            lens):
...
def cal_f1_score(preds, golds):
    ...
    for pred, gold in zip(preds, golds):
        p_sum += len(pred)
        r_sum += len(gold)
        match = 0  # To count matches for this pair
        for label in pred:
            if label in gold:
                hits += 1
        if match == len(pred) == len(gold):
            exact_matches += 1
    p = hits / p_sum if p_sum > 0 else 0
    r = hits / r_sum if r_sum > 0 else 0
    f1 = 2 * p * r / (p + r) if (p + r) > 0 else 0
    accuracy = exact_matches / len(preds) if len(preds) > 0 else 0
    print(accuracy)
    return p, r, f1
