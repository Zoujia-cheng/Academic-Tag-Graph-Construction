...
class MyModel(object):
    ...
    def __init__(self,
        ...
        bert_model2 = bert_modeling.BertModel(
            config=bert_config,
            is_training=True,
            input_ids=self.inputs_seq_word,
            input_mask=self.inputs_mask,
            token_type_ids=self.inputs_segment,
            use_one_hot_embeddings=False
        )
        ...
        bert_outputs2 = bert_model2.get_sequence_output()  # B * (S+2) * D

        embedded = tf.concat([bert_outputs, bert_outputs2], axis=2)

        with tf.variable_scope('bio_projection'):
            logits_bio = tf.layers.dense(embedded, vocab_size_bio)  # B * S * V
        ...
        with tf.variable_scope('attr_projection'):
            logits_attr = tf.layers.dense(embedded, vocab_size_attr)  # B * S * V
